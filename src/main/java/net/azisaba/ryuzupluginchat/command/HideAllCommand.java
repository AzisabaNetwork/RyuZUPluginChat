...
public class HideAllCommand implements CommandExecutor {
  ...
  public boolean onCommand(
    ...
    if (args.length >= 1
        && Arrays.asList("off", "disable", "disabled", "no")
    ...
    if (duration.isNegative()) {
      ...
      p.sendMessage(Chat.f("&c30日以上を指定することはできません"));
      return true;
    }
        ...
        .asyncFirst(
            () -> {
              try (Jedis jedis = plugin.getJedisPool().getResource()) {
                ...
                jedis.set(
                    "rpc:" + plugin.getRpcConfig().getGroupName() + ":hideall:" + p.getUniqueId(),
                    System.currentTimeMillis() + (duration.getSeconds() * 1000L) + "",
  ...
  private Duration convertStringToDuration(String str) throws IllegalArgumentException {
    ...
    while (str.length() > 0) {
      List<Integer> indexes =
          Arrays.asList(str.indexOf("d"), str.indexOf("h"), str.indexOf("m"), str.indexOf("s"));
